...
def create_course(course: CourseCreate, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
def update_course(course_id: UUID, updates: CourseUpdate, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
def delete_course(course_id: UUID, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
def enroll(course_id: UUID, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
